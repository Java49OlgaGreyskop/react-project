  ...
  const [colors, setColors]= React.useState<string[]>([])
   function creatingDivs(value: string):string{
    ...
    function getDivs(colors: string[]):JSX.Element[]{
      return colors.map(color => <div style={{width: "5vw", height: "5vh", backgroundColor: color}}></div>)
    }

  return <section style = {{display: "flex", flexDirection: "column"}}>
    <Input placeHolder ={`enter color separeted by #`} inputProcess ={creatingDivs}></Input>
